...
void app_main(void) {
  ...
  if(network_event_group == NULL) {
    ESP_LOGE(TAG, "Error: Could not intialize event group");
    abort();
  }
  ...
  // Intialize TCP/IP network interface (only call once in application)
  ...
  // Must be called prior to intializing the network driver!
  esp_ret = esp_event_loop_create_default();
  if(esp_ret != ESP_OK) {
    ESP_LOGE(TAG, "Error (%c): Failed to create default event loop", esp_ret);
    abort();
  }
